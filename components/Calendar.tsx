...
const Calendar: React.FC<CalendarProps> = ({ selectedDate, onDateSelect, appointments }) => {
  ...
  for (let d = 1; d <= totalDays; d++) {
    ...
    days.push(
      ...
      >
        {d}
        {hasAppointment(dateStr) && (
          <span className={`absolute bottom-1 w-1 h-1 rounded-full ${isSelected ? 'bg-gray-800' : 'bg-primary-dark'}`}></span>
        )}
      </button>
    );
  }
    ...
    <div className="bg-white p-4 sm:p-6 rounded-3xl shadow-sm border border-pink-50 w-full">
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex gap-2">
          <button onClick={() => changeMonth(-1)} className="p-2 hover:bg-pink-50 rounded-full transition-colors text-gray-400 hover:text-primary-dark">
      ...
      <div className="grid grid-cols-7 gap-1 text-center mb-2">
        {['D', 'S', 'T', 'Q', 'Q', 'S', 'S'].map(day => (
          <div key={day} className="text-[10px] font-bold text-gray-300 uppercase">{day}</div>
        ))}
      </div>
